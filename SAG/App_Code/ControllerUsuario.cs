...
namespace SAG.AppCode
{
    ...
    {
        readonly SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["CnnStr"].ConnectionString);
        ...
        public dynamic ActualizarPassUsuario(string Usuario, string Password)
        {
            try
            {
                string hashedPassword = BCrypt.Net.BCrypt.HashPassword(Password);
                using (SqlCommand cmd = new SqlCommand("[dbo].[ActualizarPassUsuario]", con))
                {
                    con.Open();
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    ApiRespuesta apiRespuesta = new ApiRespuesta();
                    cmd.Parameters.Add("@Usuario", System.Data.SqlDbType.VarChar).Value = Usuario;
                    cmd.Parameters.Add("@hashedPassword", System.Data.SqlDbType.VarChar).Value = hashedPassword;
